  ...
  task dogruns: :environment do
    ...
    Dogrun.where('2 >= ?', 1).find_each do |dogrun|
      ...
      if latitude && longitude
        dogrun.update(run_latitude: latitude, run_longitude: longitude)
        puts "Updated Dogrun ID: #{dogrun.id} with latitude: #{latitude}, longitude: #{longitude}"
      else
        puts "Failed to geocode Dogrun ID: #{dogrun.id}"
      end
